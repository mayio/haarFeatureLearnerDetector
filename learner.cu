...
bool loadImage(const std::string & fileName, const uint32_t loadImageIdx,
		...
		int32_t * integralImagesGpuMem, std::vector<cv::Mat> & images,
	...
	cv::cuda::GpuMat gpuIntegralImage(imageHeight, imageWidth, CV_32S,
			integralImageGpuPtr);
...
bool loadImages(int32_t * integralImagesGpuMem,
	...
	for (uint32_t i = 0;
		...
		fileError |= loadImage(fileName, loadImageIdx, imageWidth, imageHeight,
				imagesMem, imagesGpuMem, integralImagesGpuMem, images,
...
// FIMXE: set this to 40000
	...
	const cv::String pathPositiveImages =
			"/home/mona/data/objectRecognition/cars/TheKITTIVision/data/training/image_back_inline/*.png";
	const cv::String pathNegativeImages =
			"/home/mona/data/objectRecognition/cars/TheKITTIVision/data/training/negative_2/*.png";
		...
		do {
			...
			if (availableIntegralImages.size() < maxImagesPerStage
				...
				loadImages(integralImagesGpuMem, imagesToLoad, curCountImages,
						...
						imageWidth, imageHeight, falseImageStartIdx,
